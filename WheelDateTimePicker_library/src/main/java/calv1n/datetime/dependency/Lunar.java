package calv1n.datetime.dependency;
...
/**
 * 农历工具类
 * Created by duchengzhen on 2014/7/22.
 */
public class Lunar {
	...
	public final static String[] lunarNumber = {
			"零", "一", "二", "三", "四", "五", "六", "七", "八", "九"
	};
	...
	private static int getLunarMonthDays(int lunarYear, int lunarMonth) {
		// 数据表中,每个农历年用16bit来表示,
		// 前12bit分别表示12个月份的大小月,最后4bit表示闰月
		return ((Lunar.lunarInfo[lunarYear - 1900] & (0x10000 >> lunarMonth)) != 0) ? 30 : 29;
	}
	...
	private static String getLunarMonthString(int lunarMonth) {
		...
		if (lunarMonth == 1) {
			...
			if (lunarMonth % 10 > 0)
				lunarMonthString += Lunar.lunarNumber[lunarMonth % 10];
		}
	...
	public static String getLunarDayString(int lunarDay) {
		...
		String c2 = Lunar.lunarNumber[i2];
		if (lunarDay < 11) c1 = Lunar.lunarString2[0];
		if (i2 == 0) c2 = Lunar.lunarString2[1];
		return c1 + c2;
	}

	public String[] getLunarDays(int daysInLunarMonth) {
		String[] lunarDays = new String[daysInLunarMonth];
		for (int i = 1; i <= daysInLunarMonth; i++) {
			lunarDays[i] = Lunar.getLunarDayString(i);
		}
		return lunarDays;
	}

	/**
	 * 将给定的阿拉伯年份(1998)转为汉字显示
	 *
	 * @param yearNum
	 * @return 如:一九九八年
	 */
	public static String getHansYear(int yearNum) {
		if (yearNum < 1900) return "";
		int year = yearNum / 1000;
		String str = lunarNumber[year];
		year = (yearNum % 1000) / 100;
		str += lunarNumber[year];
		year = ((yearNum % 1000) % 100) / 10;
		str += lunarNumber[year];
		year = ((yearNum % 1000) % 100) % 10;
		str += lunarNumber[year];
		return str + "年";
	 ...
	 * @return 例:一九九八年 显示
	 */
	public String getLunarHansYear() {
		int num = getLunarYear();
		int year = num / 1000;
		String str = lunarNumber[year];
		year = (num % 1000) / 100;
		str += lunarNumber[year];
		year = ((num % 1000) % 100) / 10;
		str += lunarNumber[year];
		year = ((num % 1000) % 100) % 10;
		str += lunarNumber[year];
		return str + "年";
